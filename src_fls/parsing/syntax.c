...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/05 18:54:55 by ozouine           #+#    #+#             */
/*   Updated: 2025/01/05 22:05:29 by ozouine          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	q_err(t_tools *tools)
{
	...
	while (token)
	{
		...
		if (quote != 0)
		{
			ft_putstr_fd("MiniShell: Error Open Quotes !!!\n", 2);
			return (0);
		}
		token = token->next;
	}
...
static int	process_token_syntax(t_token **token)
{
	if ((is_type(*token, PIPE) && (!(*token)->prev || !(*token)->next
				|| is_type((*token)->next, PIPE) || is_types((*token)->prev,
					"TAIH"))) || (is_type(*token, PIPE) && (!(*token)->next)))
	{
		process_token_help(token);
		return (0);
	}
